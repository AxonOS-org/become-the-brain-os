...
ZIP="$(find /sdcard/Download /storage/emulated/0/Download "$HOME/storage/downloads" "$PWD" -maxdepth 2 -type f -name 'axonos-evolver-v12.1.0.zip' 2>/dev/null | head -n 1 || true)"
if [ -z "$ZIP" ]; then echo "FAIL: axonos-evolver-v12.1.0.zip not found"; exit 1; fi

WORK="$PWD/.axon-v1210-clean"
rm -rf "$WORK"
mkdir -p "$WORK"
unzip -oq "$ZIP" -d "$WORK"
PKG="$WORK/axonos-evolver-v12.1.0"
...
cp -a "$PKG/RELEASE_NOTES_v12.1.0.md" .
cp -a "$PKG/QA_MANIFEST_v12.1.0.md" .
...
echo "DONE"
echo "https://axonos-org.github.io/become-the-brain-os/evolver-evolution-version/?v=12.1.0"
