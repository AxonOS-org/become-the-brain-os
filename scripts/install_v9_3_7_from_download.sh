...
cd "${1:-$HOME/axonos-work/become-the-brain-os}"
...
ZIP="$(find /sdcard/Download /storage/emulated/0/Download "$HOME/storage/downloads" "$PWD"   -maxdepth 2 -type f -name 'axonos-evolver-v9.3.7.zip' 2>/dev/null | head -n 1 || true)"

if [ -z "$ZIP" ]; then
  echo "FAIL: axonos-evolver-v9.3.7.zip not found in Download"
  exit 1
fi

echo "FOUND: $ZIP"
WORK="$PWD/.axon-v937-clean"
rm -rf "$WORK"
mkdir -p "$WORK"
unzip -oq "$ZIP" -d "$WORK"
PKG="$WORK/axonos-evolver-v9.3.7"
...
cp -a "$PKG/RELEASE_NOTES_v9.3.7.md" .
